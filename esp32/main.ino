...
bool isValidConstructorMove(int fingerIdx, int targetAngle, int currentState[5]) {
  ...
  if (allOpen && targetAngle > 90) {
    ...
    if ((fingerIdx == 1 || fingerIdx == 3 || fingerIdx == 4) && targetAngle > 90) {
      if (tempFingers[0] < 90 && tempFingers[2] < 90) {
        return false;
      }
    }
  }
